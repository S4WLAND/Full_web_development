...
// Consultar propiedad existente
$sql = "SELECT * FROM propiedades WHERE id = ? LIMIT 1";
...
// Inicializar errores y valores con datos de la propiedad
$errores = [];
$campos = [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validación de la imagen (opcional para actualización)
    $imageName = null;
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] !== UPLOAD_ERR_NO_FILE) {
        ...
        if ($img['error'] !== UPLOAD_ERR_OK) {
            ...
            if (empty($errores)) {
                ...
                $deleteOldImage = true; // Marcar para eliminar imagen anterior
            }
        }
    }

    // Si pasa validación: actualizar registro
    if (empty($errores)) {
        // Manejar imagen: eliminar anterior y subir nueva si existe
        if ($imageName && $deleteOldImage) {
            ...
            $campos['imagen'] = $propiedad['imagen'];
        }

        // Construir y ejecutar UPDATE
        $setParts = [];
        foreach ($campos as $campo => $valor) {
            $setParts[] = "$campo = '$valor'";
        }
        $setClause = implode(', ', $setParts);
        $sql = "UPDATE propiedades SET $setClause WHERE id = $id";
...
<main class="contenedor seccion">
    ...
    <form class="formulario" method="POST" enctype="multipart/form-data">
        ...
        <fieldset>
            ...
            <?php if (!empty($campos['imagen'])): ?>
                <p class="imagen-actual">Imagen actual:</p>
                <img src="<?= IMG_BASE_URL . htmlspecialchars($campos['imagen']); ?>" alt="Imagen actual" class="imagen-small">
            <?php endif; ?>
